  ...
  return (
    <div className="p-3" style={{ background: 'var(--hud-bg-panel)', border: '1px solid var(--hud-border)' }}>
      ...
      <div className="grid grid-cols-3 gap-2 text-[13px] mb-2">
        ...
        <div><span style={{ color: 'var(--hud-primary)' }}>{formatTokens(m.input_tokens + m.output_tokens)}</span> <span style={{ color: 'var(--hud-text-dim)' }}>令牌</span></div>
      </div>
...
export default function TokenCostsPanel() {
  ...
  if (isLoading && !data) {
    return <Panel title="令牌成本" className="col-span-full"><div className="glow text-[13px] animate-pulse">正在计算成本...</div></Panel>
  }
  ...
  return (
    <>
      ...
      <Panel title={`今日 — $${today.estimated_cost_usd.toFixed(2)}`}>
        ...
        <div className="text-[13px] text-center" style={{ color: 'var(--hud-text-dim)' }}>今日估算</div>
      </Panel>
      ...
      <Panel title={`累计 — $${allTime.estimated_cost_usd.toFixed(2)}`}>
        <div className="grid grid-cols-2 gap-3 mb-3">
          ...
          <StatCard value={formatTokens(allTime.total_tokens)} label="总令牌" />
