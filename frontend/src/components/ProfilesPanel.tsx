      ...
      <div className="space-y-1 text-[13px] mb-3">
        <div className="grid grid-cols-[80px_1fr] gap-1">
          ...
          <span>
            ...
            {p.provider && <span style={{ color: 'var(--hud-text-dim)' }}> 通过 {p.provider}</span>}
          </span>
        </div>
        ...
        {p.context_length > 0 && (
          <div className="grid grid-cols-[80px_1fr] gap-1">
            ...
            <span style={{ color: 'var(--hud-text-dim)' }}>{p.context_length.toLocaleString()} 令牌</span>
          </div>
        )}

        {p.skin && (
          <div className="grid grid-cols-[80px_1fr] gap-1">
            <span style={{ color: 'var(--hud-text-dim)' }}>外观</span>
      ...
      <div className="text-[13px] mb-3 py-2" style={{ borderTop: '1px solid var(--hud-border)', borderBottom: '1px solid var(--hud-border)' }}>
        <div className="grid grid-cols-3 gap-2 mb-2">
          <div>
            ...
            <span style={{ color: 'var(--hud-text-dim)' }}> 会话</span>
          </div>
          <div>
            <span style={{ color: 'var(--hud-primary)' }} className="font-bold">{p.message_count.toLocaleString()}</span>
            <span style={{ color: 'var(--hud-text-dim)' }}> 消息</span>
          </div>
          <div>
            <span style={{ color: 'var(--hud-primary)' }} className="font-bold">{p.tool_call_count.toLocaleString()}</span>
            <span style={{ color: 'var(--hud-text-dim)' }}> 工具</span>
          </div>
        </div>
        <div className="grid grid-cols-[80px_1fr] gap-1">
          <span style={{ color: 'var(--hud-text-dim)' }}>令牌</span>
          <span style={{ color: 'var(--hud-text-dim)' }}>
            共 {formatTokens(p.total_tokens)}（输入 {formatTokens(p.total_input_tokens)} / 输出 {formatTokens(p.total_output_tokens)}）
          </span>
        </div>
        <div className="grid grid-cols-[80px_1fr] gap-1">
      ...
      <div className="space-y-1 text-[13px]">
        ...
        <div className="grid grid-cols-[80px_1fr] gap-1">
          ...
          <span><StatusDot status={p.gateway_status} /> {p.gateway_status}
            <span className="ml-3">服务 <StatusDot status={p.server_status} /> {p.server_status}</span>
          </span>
        </div>
...
export default function ProfilesPanel() {
  ...
  if (isLoading && !data) {
    return <Panel title="档案" className="col-span-full"><div className="glow text-[13px] animate-pulse">加载中...</div></Panel>
  }

  const profiles = data.profiles || []

  return (
    <Panel title={`代理档案 — 共 ${data.total || 0} 个，${data.active_count || 0} 个活跃`} className="col-span-full">
