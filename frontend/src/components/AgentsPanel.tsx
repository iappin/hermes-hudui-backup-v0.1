...
export default function AgentsPanel() {
  ...
  if (isLoading && !data) {
    return <Panel title="代理" className="col-span-full"><div className="glow text-[13px] animate-pulse">正在扫描进程...</div></Panel>
  }
          ...
          {live.map((proc: any, i: number) => (
            <div key={`${proc.name}-${proc.pid}-${i}`} className="p-2" style={{ background: 'var(--hud-bg-panel)', borderLeft: '3px solid var(--hud-success)' }}>
              <div className="flex items-center gap-2 text-[13px]">
                ...
                <span className="text-[13px]" style={{ color: 'var(--hud-success)' }}>运行中</span>
        ...
        {data.has_tmux && tmuxPanes.length > 0 && (
          <div className="mt-3 text-[13px]">
            <div className="uppercase tracking-wider mb-1" style={{ color: 'var(--hud-text-dim)' }}>
              tmux 窗格 — 共 {tmuxPanes.length} 个，已映射 {data.matched_pane_count} 个
            </div>
