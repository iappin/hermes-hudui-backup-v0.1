...
export default function CorrectionsPanel() {
  ...
  return (
    <Panel title={`纠错与经验 — 共 ${corrections.length} 条`} className="col-span-full">
      {/* Summary */}
      <div className="flex gap-4 text-[13px] mb-3">
        {['critical', 'major', 'minor'].map(sev => {
          ...
          return (
            <span key={sev}>
              <span style={{ color: s.color }}>{s.icon} {count} {SEVERITY_LABELS[sev]}</span>
            </span>
          )
        })}
        {corrections.length === 0 && (
          <span style={{ color: 'var(--hud-text-dim)' }}>还没有纠错记录。这不是很厉害，就是有点可疑。</span>
        )}
      </div>
      ...
      {corrections.length > 0 && (
        <div className="text-[13px] italic mb-3" style={{ color: 'var(--hud-text-dim)' }}>
          这里记录的是我出错、被纠正，或吃过苦头后学到的东西。严重 = 用户发现明确错误。重要 = 已吸收的坑点。轻微 = 已记录的限制。
        </div>
      )}
        ...
        return (
          <div key={sev} className="mb-4">
            <div className="text-[13px] font-bold mb-2" style={{ color: s.color }}>
              {s.icon} {SEVERITY_LABELS[sev]} ({items.length})
            </div>
                  ...
                  <div className="flex items-center gap-2 text-[13px] mb-1">
                    ...
                    {cor.timestamp && (
                      <span style={{ color: 'var(--hud-text-dim)' }}>
                        {new Date(cor.timestamp).toLocaleDateString('zh-CN')} {new Date(cor.timestamp).toLocaleTimeString('zh-CN')}
                      </span>
                    )}
