...
export default function CronPanel() {
  ...
  if (!Array.isArray(jobs) || jobs.length === 0) {
    return <Panel title="定时任务" className="col-span-full"><div className="text-[13px]" style={{ color: 'var(--hud-text-dim)' }}>还没有配置定时任务</div></Panel>
  }
            ...
            <div key={job.id} className="p-3" style={{ background: 'var(--hud-bg-panel)', border: '1px solid var(--hud-border)' }}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[13px] px-1.5 py-0.5 ml-auto"
                  ...
                  {job.state === 'scheduled' ? '已计划' : job.state || '未知'}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-[13px]">
                ...
                <div>
                  ...
                  <div>{job.next_run_at ? new Date(job.next_run_at).toLocaleString('zh-CN') : '-'}</div>
                </div>
                <div>
              ...
              {job.repeat_completed != null && (
                <div className="mt-2 text-[13px]" style={{ color: 'var(--hud-text-dim)' }}>
                  已完成运行：{job.repeat_completed}{job.repeat_total ? ` / ${job.repeat_total}` : ''}
