...
from backend.collectors.health import collect_health, last_network_status
...
async def get_dashboard():
    ...
    state = collect_all()
    health = collect_health(include_network=False)
    health.network = last_network_status()
    ...
    # Projects: keep the full collector result so the overview matches the
    # dedicated Projects tab instead of silently hiding quiet repositories.
    projects_data = collect_projects()
    projects = {
        "projects": to_dict(projects_data.projects),
